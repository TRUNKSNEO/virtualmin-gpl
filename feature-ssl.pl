...
# feature_depends_ssl()
# Return undef if SSL feature prerequisites are met
sub feature_depends_ssl
{
my $werr = &feature_depends_web();
return $werr if ($werr);
$config{'web'} || return &text('check_edepssl', "edit_newfeatures.cgi");
return undef;
}
...
if ($d->{'auto_redirect'} // $tmpl->{'web_sslredirect'}) {
